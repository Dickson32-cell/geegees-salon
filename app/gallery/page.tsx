            ...
            return (
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-4 md:p-6 pointer-events-none">
                  ...
                  <div className="mt-2 md:hidden flex items-center gap-2 text-secondary text-xs font-bold">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                    </svg>
                    <span>Tap anywhere to view fullscreen</span>
                  </div>
                </div>

                {/* Clickable overlay for mobile - ensures taps work */}
                <div className="absolute inset-0 md:hidden" onClick={() => setSelectedImage(item)}></div>
              </div>
            );
          })}
        </div>
