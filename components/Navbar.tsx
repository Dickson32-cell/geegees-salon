...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* Mobile Menu Drawer */}
      <div
        className={`fixed inset-0 bg-black/60 z-[55] md:hidden transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
        onClick={() => setIsMobileMenuOpen(false)}
      >
        <div
          className={`absolute left-0 top-0 h-full w-4/5 max-w-sm bg-surface shadow-2xl p-6 overflow-y-auto transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
            }`}
